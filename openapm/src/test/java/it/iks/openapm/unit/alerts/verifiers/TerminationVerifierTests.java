...
@SpringBootTest()
public class TerminationVerifierTests {
    @MockBean
    ...
    private AlertConfig config() {
        ...
        config.setVerification(new VerificationConfig("termination", 0));
        return config;
    }

    @Test
    public void it_fires_event_on_inactive() {
    ...
    @Test
    public void it_does_not_fire_event_on_active() {
        AlertConfig config = config();
        Verifier verifier = factory.forAlert(config);
